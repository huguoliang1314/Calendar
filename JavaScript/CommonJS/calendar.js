...
var calendar = (function () {
    ...
    var convertMonth = function (argument, index) {
        ...
        //index=1,表示向后一个月
        if (index === 1 && Number.isInteger(argument)) {
    ...
    var matrix = function () {
        ...
        var str = '<div class="month-row1"><span class="month col1">{0}</span><span class="month col2">{1}</span><span class="month col3">{2}</span><span class="month col4">{3}</span></div>'
            + '<div class="month-row2"><span class="month col1">{4}</span><span class="month col2">{5}</span><span class="month col3">{6}</span><span class="month col4">{7}</span></div>'
            + '<div class="month-row3"><span class="month col1">{8}</span><span class="month col2">{9}</span><span class="month col3">{10}</span><span class="month col4">{11}</span></div>';
    ...
    return {
        ...
        searchPreYear: function () {
            let currentYear = document.querySelector('.date-month');
            let year = parseInt(currentYear.textContent);
            currentYear.textContent = --year;
        },
        searchNextYear: function () {
            let currentYear = document.querySelector('.date-month');
            let year = parseInt(currentYear.textContent);
            currentYear.textContent = ++year;
        },
        searchPosition: function (year, argument) {
            let month = convertMonth(argument, 0);
            let firstDate = new Date(year, convertNum(month)),
                firstWeek = firstDate.getDay();
            findPostion(firstWeek, convertNum(month));
            dateTarget.textContent = argument + " " + year;
        }
    }
...
prevTarget.addEventListener("click", function () {
    let content = dateTarget.textContent;
    let re = /\w+\s/g;
    let array = content.match(re);
    if (array === null) {
        calendar.searchPreYear();
    }
    else {
...
nextTarget.addEventListener("click", function () {
    let integer = parseInt(dateTarget.textContent);
    if (Number.isInteger(integer)) {
        calendar.searchNextYear();
    } else {
...
dateTarget.addEventListener("click", function (element) {
    var monthArr = document.getElementsByClassName('row9');
    if (monthArr.length === 0) {
        var data = new Date();
        dateTarget.textContent = data.getFullYear();
    ...
    Array.from(mousetarget).forEach(function (element) {
        ...
        element.addEventListener('click', ()=> {
            let year = parseInt(dateTarget.textContent),
                month = element.textContent;
            calendar.searchPosition(year, month);

            let tbody = document.querySelector('.table tbody'),
                table = document.querySelector('.table'),
                trow = document.querySelector('.row9');
            tbody.style.display = '';
